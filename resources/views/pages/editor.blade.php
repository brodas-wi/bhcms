...
<html lang="es">
    ...
    <body>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" id="exit-version-type">
                                ...
                                <option value="current">Versión actual ({{ $page->version ?? '1.0.0' }})</option>
                                <option value="minor">Versión menor
                                    ({{ $page->version ? explode('.', $page->version)[0] . '.' . (explode('.', $page->version)[1] + 1) . '.0' : '1.1.0' }})
                                </option>
                                <option value="major">Versión mayor
                                    ({{ $page->version ? explode('.', $page->version)[0] + 1 . '.0.0' : '2.0.0' }})
                                </option>
                            </select>
                        </div>
                    </div>
        ...
        <script>
            var availablePlugins = @json($availablePlugins);
            var serializedContent = @json($serializedContent);
            var activePlugins = @json($activePlugins);
        </script>
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                function openInternalLinkModal(component) {
                    ...
                    component.set('attributes', {
                        href: '{{ route('pages.display', ':slug') }}'.replace(':slug', selectedPageSlug)
                    });
                }
                ...
                editor.Commands.add('save-page', {
                    run: function(editor) {
                        ...
                        document.getElementById('confirm-save').onclick = function() {
                            ...
                            fetch('{{ isset($page->id) ? route('pages.update', $page->id) : route('pages.store') }}', {
                ...
                editor.on('load', () => {
                    ...
                    editor.setStyle({!! json_encode($defaultStyles) !!});

                    // Si hay contenido serializado, cárgalo
                    @if (isset($page->serialized_content))
                        const components = {!! $page->serialized_content !!};
